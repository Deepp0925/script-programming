...
class Firewall(object):
    ...
    def inspectPacket(self, netPacket):
        ...
        if action == "Block":
            ...
            print("-", netPacket.src, "to", netPacket.dst,
                  "using port", netPacket.port, "is blocked")
        elif action == "Allow":
            if self.verbose == True:
                print("+", netPacket.src, "to", netPacket.dst,
                      "using port", netPacket.port, "is allowed.")
            self.totalPacketsAllowed = self.totalPacketsAllowed + 1
        else:
            if self.verbose == True:
                print("+", netPacket.src, "to", netPacket.dst, "using port",
                      netPacket.port, "is allowed due to no rule.")
            self.totalPacketsAllowed = self.totalPacketsAllowed + 1
    ...
    def findRuleMatch(self, netPacket):
        for rule in self.rules:
            if netPacket.src == rule["src"] and netPacket.dst == rule["dst"] and netPacket.port == rule["port"]:
                return rule["action"]
            # Your CODE HERE
            elif rule["src"] == "*" and rule["dst"] == "*" and rule["port"] == netPacket.port:
                return rule["action"]
            else:
                continue
